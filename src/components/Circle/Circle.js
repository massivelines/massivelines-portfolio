...
const transform = () => {
  ...
  return `translate(0, ${translateY}),scale(1, ${scaleY})`;
...
const animate = (path, translateY, scaleY) => {
  ...
  const newTrans = randomTranslate();
...
class Circle extends Component {
  ...
  componentDidMount() {
    ...
    this.SvgRef.current.childNodes[0].childNodes.forEach(element => {
      ...
      const transRegex = /(?<=\(0, )(.*)(?=\),)/;
      const translateY = value.match(transRegex);
      const scaleRegex = /(?<=\(1, )(.*)(?=\))/;
      const scaleY = value.match(scaleRegex);

      animate(element, translateY[0], scaleY[0]);
    });
  }
    ...
    return (
      <div className="circle">
        <svg
          ...
          viewBox="0 0 768 768"
        >
          <g className="svg-color" fillRule="evenodd">
            <path
          ...
          {/* <path fill="none" d="M0 0h768v768H0z" style={style()} /> */}
        </svg>
      </div>
    );
  }
